...
int row, col, tok[ 12 ][ 1010 ], X, mamo[ 1010 ][ 15 ];
...
void input()
{
    ...
    while( T-- )
    {
        ...
        col = X / 100, row = 10;
        for(int i = 9; i > -1; i--)
            for(int j = 0; j < col; j++) sf("%d", &tok[ i ][ j ]);
        CLR(mamo, -1);
        pf("%d\n\n", DP(0, 0));
    }
}
int DP(int pos, int state)
{
    if(pos > col || state < 0 || state > 9) return INF;
    if(pos == col)
    {
    ...
    if(ret != -1) return ret;
        ...
        int tmp = tok[ state ][ pos ];
        int up = 60, dwn = 20, flat = 30;

            up = 60 - tmp;
            dwn = 20 - tmp;
            flat = 30 - tmp;

    return ret = min(up + DP(pos + 1, state + 1),
            min(flat + DP(pos + 1, state), dwn + DP(pos + 1, state - 1)));
}
