...
long long int tollInt(string s){long long r=0; istringstream sin(s); sin>>r; return r;}
...
lld DP(int pos, int issmall, lld val);
...
lld DP(int pos, int issmall, lld val)
{
    ...
    {
        mark[ in[ pos ] - 48 ]++;
        ret = max(ret, DP(pos + 1, 0, val * 10 + (in[ pos ] - 48)));
        mark[ in[ pos ] - 48 ]--;

        for(int i = in[ pos ] - 49; i > 0; i--)
        {
            mark[ i ]++;
            ret = max(ret, DP(pos + 1, 1, val * 10 + i));
            mark[ i ]--;
        }
    }
    else
