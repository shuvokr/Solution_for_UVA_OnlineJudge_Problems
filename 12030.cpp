...
lld DP(int pos, int mask, int flag);

int n, in[ 17 ][ 17 ];
lld mamo[ 16 ][1 << 15][ 3 ], mark[ 16 ][1 << 15][ 3 ], ck;
...
lld DP(int pos, int mask, int flag)
{
    ...
    lld &ret = mamo[ pos ][ mask ][ flag ];
    if(mark[ pos ][ mask ][ flag ] == ck) return ret;
    mark[ pos ][ mask ][ flag ] = ck;
    ret = 0;
    for(int i = 0; i < n; i++)
    {
        if(check(mask, i)) continue;
        if(flag == 2) ret += DP(pos + 1, biton(mask, i), flag);
        else if(in[ pos ][ i ] == 0) ret += DP(pos + 1, biton(mask, i), 1);
        else if(in[ pos ][ i ] == 2) ret += DP(pos + 1, biton(mask, i), 2);
        else ret += DP(pos + 1, biton(mask, i), flag);
    }
    return ret;
}
