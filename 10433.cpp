...
int main() {
    ...
    while(~scanf("%s",inp)){
        ...
        int k;
        for(k=len-1;k>0;k--)if(x[k])break;
        if(k==0&&(x[k]==1||x[k]==0)){
            puts("Not an Automorphic number.");
            continue;
        }
...
int main() {
    ...
    // taking Bigint input //
    /////////////////////////
    string input, next; // string to take input

    while(cin >> input)
    { // take the Big integer as string
        ...
        int i = 0;
        while(input[ i ] == '0' && i < len) i++;
        if(i == len || (input[ i ] == '1' && i + 1 == len))
        {
            puts("Not an Automorphic number.");
            continue;
        }
        ...
        {
            k = 0;
            for(int i = len - 1000; i < len; i++) input[ k++ ] = input[ i ];
            input[ k ] = 0;
        }
        len = k;
        a = input;
        ...
        int x = 0, y = input.size() - 1, res = 0;

        while( next[ x ] == input[ y ] && y > -1 )
        {
            res++;
            x++; y--;
        }

        if( res ) printf("Automorphic number of %d-digit.\n", res);
        else puts("Not an Automorphic number.");
    }
    return 0;
}
